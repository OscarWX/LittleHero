...
export function createExampleBooks(profileId: string) {
  ...
  const books: BookData[] = existingBooksJson ? JSON.parse(existingBooksJson) : []

  // Only add example books if there are none for this profile
  const hasProfileBooks = books.some((book) => book.profileIds.includes(profileId))
  ...
  const readyBook: BookData = {
    ...
    magicalDetails: "Emma discovers a magical key that opens doors to other worlds.",
    ...
    status: "ready",
    pictures: ["/child-and-fox.png", "/child-and-fox-sky.png", "/child-and-fox-wings.png"],
