...
export default function StoryThemePage() {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      const theme = selectedTheme === "personalized" ? customTheme : themeOptions.find((t) => t.id === selectedTheme)?.label || ""
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to save theme: ${response.status} ${response.statusText} - ${errorData.error || 'Unknown error'}`)
      }
      ...
      alert(`Error saving theme: ${error instanceof Error ? error.message : 'Unknown error'}`)
      setIsLoading(false)
    }
  }
        ...
        <div className="max-w-md mx-auto">
            ...
            {themeOptions.map((theme) => (
              <button
                ...
                className={`p-4 rounded-lg text-center transition-colors ${
                  selectedTheme === theme.id ? "border-2 border-yellow-500" : "border border-gray-200 hover:bg-gray-50"
                }`}
                style={{ backgroundColor: selectedTheme === theme.id ? `${theme.color}20` : "white" }}
          ...
          {selectedTheme === "personalized" && (
            <div className="bg-white p-4 rounded-lg mb-8 border border-gray-200">
              <label htmlFor="customTheme" className="block text-gray-800 nunito-font font-bold mb-2">
          ...
          <button
            onClick={handleNext}
            disabled={selectedTheme === "" || (selectedTheme === "personalized" && customTheme === "")}
