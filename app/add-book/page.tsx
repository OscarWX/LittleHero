...
export default function AddBookPage() {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      const characterNames = selectedProfiles.map(id => {
        const profile = profiles.find(p => p.id === id)
        return profile?.name || ""
      }).filter(Boolean)
    ...
    <div className="flex flex-col min-h-screen bg-[#FFF8E8]">
      ...
      <main className="flex-1 p-6">
        <div className="max-w-md mx-auto">
          <h2 className="text-2xl font-bold text-gray-900 mb-6 junegull-font text-center">CHOOSE YOUR CHARACTERS</h2>
          ...
          {selectedProfiles.length > 0 && (
            <div className="bg-white p-4 rounded-lg mb-8 text-center">
              <p className="text-gray-800 nunito-font">
                This is a story about <span className="font-bold">{formatCharacterList()}</span>
              </p>
            </div>
          )}
