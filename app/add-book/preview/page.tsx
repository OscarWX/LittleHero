      ...
      try {
        ...
        if (bookData.status === 'creating' || !bookData.story_content) {
          ...
          const { book: updatedBook } =
            await bookApi.generateStory(bookIdParam);
          setBook(updatedBook);
        ...
        <div className='flex-1 flex items-center justify-center'>
          <div className='text-center max-w-md mx-auto p-6'>
            ...
            <p className='text-gray-700 nunito-font mb-4'>
              Our AI is crafting a personalized story based on your
              selections...
            </p>
          ...
          <div className='max-w-md mx-auto text-center'>
            ...
            <div className='bg-white p-6 rounded-lg shadow-sm mb-8'>
              <p className='text-gray-800 nunito-font mb-4'>
                Your personalized story has been created! Pictures are being
                prepared for each page. You can view your story in the
                dashboard.
              </p>
              <div className='flex items-center justify-center space-x-2 text-yellow-600 nunito-font'>
                <BookOpen size={18} />
                <span className='font-bold'>
                  {story?.title || 'Your Adventure Story'}
                </span>
              </div>
            </div>
            <Link
  ...
  if (!story) {
    return (
      <div className='flex flex-col min-h-screen bg-[#FFF8E8]'>
        <div className='flex-1 flex items-center justify-center'>
          <div className='text-center'>
            <p className='text-gray-700 nunito-font'>
  ...
  return (
    <div className='min-h-screen bg-[#FFF8E8]'>
      ...
      <header className='p-4 flex items-center'>
        <Link
          href={`/add-book/narrative-style?bookId=${bookId}`}
      ...
      <main className='max-w-md mx-auto px-4 pb-24'>
        <h2 className='text-2xl font-bold text-center mb-2 junegull-font'>
          {story.title}
        </h2>
        <p className='text-center text-gray-600 nunito-font mb-6'>
          A story featuring{' '}
          {book?.child_profiles.map(p => p.name).join(' and ')}
        </p>

        <div className='bg-yellow-50 rounded-lg p-6 mb-6'>
          {story.pages.map((page, i) => (
            <div key={i} className='mb-4 flex gap-2'>
              <div className='text-yellow-600 font-bold nunito-font'>
                p{page.pageNumber}
              </div>
