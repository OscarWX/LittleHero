...
export default function BookPreviewPage() {
  ...
  const getCharacterNames = () => {
    ...
    const bookCharacters = profiles.filter(profile =>
      book.profileIds.includes(profile.id)
    );
  ...
  const handleDeleteBook = () => {
    ...
    if (savedBooks) {
      const books = JSON.parse(savedBooks);
      const filtered = books.filter((b: BookData) => b.id !== bookId);
    ...
    try {
      ...
      if (savedBooks) {
        ...
        if (bookIndex !== -1) {
          ...
          setBook({ ...books[bookIndex] }); // update local state
        }
      }
    } catch (e) {
      console.error('Error updating book status to ready', e);
    }
  };
      ...
      <div className='flex flex-col min-h-screen bg-[#FFF8E8]'>
        <main className='flex-1 px-4 flex flex-col items-center justify-center'>
          <div className='max-w-md mx-auto text-center'>
            ...
            <h2 className='text-3xl font-bold text-gray-900 mb-4 junegull-font'>
              REGENERATING...
            </h2>
            <div className='bg-white p-6 rounded-lg shadow-sm mb-8'>
              <p className='text-gray-800 nunito-font mb-4'>
                Your book is being regenerated! We're creating new pictures for
                each page.
              </p>
              <p className='text-gray-800 nunito-font mb-4'>
  ...
  return (
    <div className='flex flex-col min-h-screen bg-[#FFF8E8]'>
      <header className='p-4 flex items-center justify-between relative'>
        ...
        {showMenu && (
          <div className='absolute right-4 top-14 bg-white shadow-lg rounded-lg w-48 z-50'>
            <button
              onClick={handleEditBook}
              className='w-full text-left px-4 py-2 hover:bg-gray-100 flex items-center gap-2'
            >
              <Edit size={16} /> Jump to Ready
            </button>
            <button
      ...
      <div className='px-4 mb-4'>
        <div className='bg-yellow-100 rounded-lg p-3 text-center'>
          <p className='text-gray-700 nunito-font text-sm'>
            A story about{' '}
            <span className='font-bold'>{getCharacterNames()}</span>
          </p>
        </div>
      </div>
      ...
      <main className='flex-1 px-4 pb-24'>
        <div className='max-w-md mx-auto'>
          ...
          {book.status === 'ready' && (
            <Link
              href={`/book/${bookId}/order`}
              className='flex-1 py-3 px-4 bg-green-500 hover:bg-green-600 text-white font-bold rounded-lg transition-colors duration-300 nunito-font text-sm flex items-center justify-center gap-2'
            >
              <Printer size={16} />
          ...
          <div className='space-y-8 mb-8'>
            {book.content.map((paragraph, index) => (
              <div
                key={index}
                className='bg-yellow-50 rounded-lg p-6 shadow-sm'
              >
          ...
          <div className='bg-white rounded-lg p-6 shadow-sm mb-8'>
            <h3 className='text-xl font-bold text-gray-900 mb-4'>
              {book.status === 'ready'
                ? 'Add Feedback to Regenerate Storybook'
                : 'Add Feedback to Regenerate Story'}
            </h3>
            <p className='text-gray-700 nunito-font mb-4'>
              Want to make the story even better? Tell us what you'd like
              changed — such as tone, plot, images, or anything else.
            </p>
            <textarea
          ...
          <div className='max-w-md mx-auto px-4 pb-12 flex flex-col gap-4'>
            ...
            <Link
              href='/dashboard?tab=books'
              className='w-full text-center py-3 px-4 bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold rounded-full transition-colors duration-300 nunito-font'
            >
              Go back to Home
            </Link>
          </div>
        ...
        <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4'>
          <div className='bg-white rounded-lg p-6 max-w-sm w-full'>
            ...
            <p className='text-gray-700 nunito-font mb-6'>
              Are you sure you want to regenerate this story? This might take
              some time, and the current pictures will be replaced.
            </p>
