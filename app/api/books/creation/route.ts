...
import { createDraftBook, type BookCreationData } from '@/lib/db/books';
  ...
  try {
    ...
    if (fetchUpdatedError) {
      ...
      return NextResponse.json(
        {
          error:
            'Book updated but failed to fetch: ' + fetchUpdatedError.message,
        },
        { status: 500 }
      );
    }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (creationData) {
      ...
      updateData.special_memories_image_url =
        creationData.specialMemoriesImageUrl;
